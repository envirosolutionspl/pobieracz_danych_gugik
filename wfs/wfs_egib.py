                    ...
                    with get_legacy_session().get(url_gml, verify=False, timeout=60) as resp:
                        if str(resp.status_code) == '404':
                            ...
                            with open(folder + teryt + '_' + layer.split(':')[-1] + '_egib_wfs_gml.gml', 'wb') as f:
                                f.write(resp.content)
                                # Otwórz istniejący plik GML
                                ds = ogr.Open(f, update=True)
                                if ds is None:
                                    print("Nie udało się otworzyć pliku GML.")
                                    exit()

                                # Pobierz pierwszą warstwę (jeśli plik GML zawiera więcej niż jedną warstwę, dostosuj ten krok)
                                layer = ds.GetLayer()

                                # Utwórz obiekt SpatialReference dla układu współrzędnych, którego chcesz użyć
                                output_srs = osr.SpatialReference()
                                output_srs.ImportFromEPSG(2180)  # WGS84

                                # Ustaw układ współrzędnych projektu dla warstwy istniejącej
                                layer.SetSpatialRef(output_srs)
                                name_error = "brak"
