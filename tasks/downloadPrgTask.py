...
class DownloadPrgTask(QgsTask):
    ...
    def run(self):
        ...
        is_success, message = self.service_api.retreiveFile(
            url=self.url,
    ...
    def finished(self, result):
        if result:
            ...
            MessageUtils.pushSuccess(
                self.iface,
                "Udało się! Dane PRG zostały pobrane."
            )
        else:
            if self.exception and self.exception is not True:
                error_msg = str(self.exception)
            else:
                error_msg = "Błąd nieznany"
            MessageUtils.pushLogInfo(
                f"Nie udało się pobrać danych PRG. Wystąpił błąd: {error_msg}"
            )
